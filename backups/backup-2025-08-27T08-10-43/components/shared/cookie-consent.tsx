...
import { consentManager, type CookieConsent } from '@/lib/analytics/consent-manager'
...
export function CookieConsentBanner() {
  ...
  const saveConsent = (consentData: CookieConsent) => {
    // Save consent using the consent manager
    consentManager.saveConsent(consentData)
    setShowBanner(false)

    // Reload analytics with new consent
          ...
          {!showDetails ? (
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-start justify-between p-3 border rounded-lg">
                <div className="flex-1 space-y-1">
                  <div className="flex items-center gap-2">
                    <Cookie className="h-4 w-4 text-workflo-yellow" />
                ...
                <label className="ml-3 flex items-center cursor-pointer">
                  <input
                    ...
                    className="sr-only"
                  />
                  <div className={`w-4 h-4 rounded-full border-2 ${consent.preferences ? 'bg-workflo-yellow border-workflo-yellow' : 'border-gray-300'}`}></div>
                </label>
              </div>
