/**
 * Script to upload news articles to blog CMS via API
 *
 * Run with: npx tsx scripts/upload-blog-articles-api.ts
 *
 * NOTE: You need to be logged in to the admin panel to get a valid token.
 * 1. Go to http://localhost:3000/admin/login
 * 2. Log in
 * 3. Open browser console and run: localStorage.getItem('admin_token')
 * 4. Copy the token and set it as ADMIN_TOKEN environment variable
 *
 * Then run: ADMIN_TOKEN=fFlH4rqhvQrRnScrZQnYW5rYYaRmYvYoLbiFRWjImy7v1m9OEuUbicgkfvJMGWW9 npx tsx scripts/upload-blog-articles-api.ts
 */
...
async function uploadArticles() {
  ...
  if (!token) {
    ...
    console.error('1. Go to http://localhost:3000/admin/login')
    console.error('2. Log in to the admin panel')
    console.error('3. Open browser console and run: localStorage.getItem("admin_token")')
    console.error('4. Copy the token and run this script with:')
    console.error('   ADMIN_TOKEN=your_token_here npx tsx scripts/upload-blog-articles-api.ts\n')
    process.exit(1)
  }

  console.log('Starting blog article upload via API...\n')
  ...
  for (const article of articles) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.log(`  Tags: ${tags.join(', ')}, Read time: ${readTime} min\n`)
        successCount++
      }
    } catch (error) {
